...
bool
aarch64_pe_type_dllimport_p (tree decl)
{
...
bool
aarch64_pe_type_dllexport_p (tree decl)
{
  gcc_assert (TREE_CODE (decl) == VAR_DECL 
              || TREE_CODE (decl) == FUNCTION_DECL);
...
{
  if (aarch64_pe_type_dllimport_p (decl))
    DECL_DLLIMPORT_P (decl) = 1;
}

static inline void maybe_add_dllexport (tree decl) 
{
  if (aarch64_pe_type_dllexport_p (decl))
    {   
...
void
aarch64_pe_adjust_class_at_definition (tree t)
{
  tree member;
